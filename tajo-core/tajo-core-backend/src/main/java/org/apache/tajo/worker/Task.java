...
public class Task {
  ...
  public Task(QueryUnitAttemptId taskId, final WorkerContext worker, final Interface masterProxy,
	...
	this.context = new TaskAttemptContext(conf, taskId, request.getFragments().toArray(
	...
	plan = GsonCreator.getInstance().fromJson(request.getSerializedData(), LogicalNode.class);
	...
	if (interQuery) {
	  ...
	  // the task id.
	  Path outFilePath = new Path(conf.getOutputPath(), OUTPUT_FILE_PREFIX
  ...
  public void run() {
	...
	try {
	  ...
	  if (context.getFragmentSize() > 0) {
		this.executor = workerContext.getTQueryEngine().createPlan(context, plan);
		this.executor.init();
  ...
  private List<Fetcher> getFetchRunners(TaskAttemptContext ctx, List<Fetch> fetches) throws IOException {
	...
	if (fetches.size() > 0) {
	  Path inputDir = lDirAllocator.getLocalPathToRead(getTaskAttemptDir(ctx.getTaskId()).toString() + "/in", conf);
	  File storeDir;
  ...
  public String getFileCacheDir() {
	fileCache = USERCACHE + "/" + "hyunsik" + "/" + APPCACHE + "/"
		+ ConverterUtils.toString(taskId.getQueryId().getApplicationId()) + "/" + "output";
	return fileCache;
  }

  public static Path getTaskAttemptDir(QueryUnitAttemptId quid) {
	Path workDir = StorageUtil.concatPath(quid.getSubQueryId().toString(),
