...
public class Query implements EventHandler<QueryEvent> {
  ...
  public Query(final QueryContext context, final QueryId id, Clock clock, final long appSubmitTime,
	  final String queryStr, final EventHandler eventHandler, final MasterPlan plan, final StorageManager sm) {
	  ...
	  if (castEvent.getFinalState() == SubQueryState.SUCCEEDED) {
		if (cursor.hasNext()) {
		  ...
		  if (query.checkQueryForCompleted() == QueryState.QUERY_SUCCEEDED) {
			...
			TableDesc desc = new TableDescImpl(query.conf.getOutputTable(), subQuery.getTableMeta(),
				query.context.getOutputPath());
  ...
  private void writeStat(Path outputPath, SubQuery subQuery) throws IOException {
	...
	if (execBlock.getPlan().getType() == ExprType.CREATE_INDEX) {
	  ...
	  if (sm.getFileSystem().exists(new Path(indexPath, ".meta"))) {
		...
		meta = CatalogUtil.newTableMeta(execBlock.getOutputSchema(), StoreType.CSV);
	  }
	  String indexName = IndexUtil.getIndexName(index.getTableName(), index.getSortSpecs());
