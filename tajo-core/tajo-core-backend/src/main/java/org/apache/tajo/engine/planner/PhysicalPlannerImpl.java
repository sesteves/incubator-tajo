...
public class PhysicalPlannerImpl implements PhysicalPlanner {
  ...
  public PhysicalExec createJoinPlan(TaskAttemptContext ctx, JoinNode joinNode, PhysicalExec outer, PhysicalExec inner)
    ...
    default:
      SortSpec[][] sortSpecs = PlannerUtil.getSortKeysFromJoinQual(joinNode.getJoinQual(), outer.getSchema(),
          inner.getSchema());
      ExternalSortExec outerSort = new ExternalSortExec(ctx, sm, new SortNode(sortSpecs[0], outer.getSchema(),
          outer.getSchema()), outer);
      ExternalSortExec innerSort = new ExternalSortExec(ctx, sm, new SortNode(sortSpecs[1], inner.getSchema(),
          inner.getSchema()), inner);
  ...
  public PhysicalExec createIndexWritePlan(StorageManager sm, TaskAttemptContext ctx, IndexWriteNode indexWriteNode,
      PhysicalExec subOp) throws IOException {

    return new IndexWriteExec(ctx, sm, indexWriteNode, ctx.getTable(indexWriteNode.getTableName()), subOp);
  }
